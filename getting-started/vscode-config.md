...
> 📷 **截图占位**：VS Code 终端下拉菜单中出现 "MSYS2 UCRT"（待补充）
...
> 📷 **截图占位**：红线报错的快速修复菜单（待补充）

## 5. 配置 CMake Tools

第一次打开带 `CMakeLists.txt` 的项目文件夹时，CMake Tools 会引导你：

1. 底部状态栏点「No Kit Selected」，选择 **GCC for x86_64-w64-mingw32（UCRT64）** 对应的 kit
2. 生成器选 **Ninja**（如果询问的话）
...
下一步 → [验证与排坑](verify-and-troubleshoot.md)
